...
(()=>{
  ...
  document.querySelectorAll('.footer-bottom span:last-child').forEach(el=>{
    if(el.textContent.includes('Build v0.'))el.textContent='Build v0.4 · September 2026';
  });

  const steps=`<div><i>01</i><b>Defined population</b><small>Know who the system is responsible for.</small></div><div><i>02</i><b>Accountable front-line team</b><small>Continuity begins close to home.</small></div><div><i>03</i><b>Reliable primary care</b><small>Peripheral Health Unit services must work consistently.</small></div><div><i>04</i><b>Higher-capacity support</b><small>Diagnostics and escalation support.</small></div><div><i>05</i><b>Formal referral</b><small>Higher-level care stays connected.</small></div><div><i>06</i><b>Documented return</b><small>Information and follow-up close the loop.</small></div>`;
  const title=document.title;
  const pageHero=document.querySelector('.page-hero');

  if(title.startsWith('A more reliable')){
    const diagram=document.querySelector('.hero-diagram');
    if(diagram)diagram.innerHTML=`<div class="diagram-label">Model 0.x · system responsibility</div><div class="system-schematic">${steps}</div>`;
    const preview=document.querySelector('.model-preview');
    if(preview){
      const board=document.createElement('div');
      board.className='container version-board';
      board.innerHTML=`<div class="version-id"><small>Current public model</small><strong>0.x</strong><small>Research & Co-Design</small></div><div class="version-data"><div><small>Decision rule</small><b>Evidence can change the model</b></div><div><small>Audit trail</small><b>Material changes remain visible</b></div><div><small>Next questions</small><b>Cost, workforce and implementation conditions</b></div></div>`;
      preview.after(board);
    }
  }

  if(title.startsWith('The evolving model')&&pageHero){
    const section=document.createElement('section');
    section.className='section border-top';
    section.innerHTML=`<div class="container"><div class="version-board"><div class="version-id"><small>Current model</small><strong>0.x</strong><small>Research & Co-Design</small></div><div class="version-data"><div><small>Implementation</small><b>Not approved for clinical delivery</b></div><div><small>Change rule</small><b>Evidence outranks original preference</b></div><div><small>Next state</small><b>Technical design only after research gates close</b></div></div></div><div style="height:22px"></div><div class="system-schematic">${steps}</div></div>`;
    pageHero.after(section);
  }

  if(title.startsWith('Maternal Health Stress Test')&&pageHero){
    const section=document.createElement('section');
    section.className='section border-top';
    section.innerHTML=`<div class="container"><p class="eyebrow">End-to-end tracer</p><h2>One weak link can break the entire pathway.</h2><div class="system-schematic"><div><i>01</i><b>Pregnancy identified</b><small>Continuity starts early.</small></div><div><i>02</i><b>Antenatal continuity</b><small>Routine monitoring stays connected.</small></div><div><i>03</i><b>Risk recognized</b><small>Danger is identified before crisis.</small></div><div><i>04</i><b>Referral + transport</b><small>Escalation reaches a receiving facility.</small></div><div><i>05</i><b>Emergency capability</b><small>The receiving level can respond.</small></div><div><i>06</i><b>Return + follow-up</b><small>Mother and newborn return to accountable care.</small></div></div></div>`;
    pageHero.after(section);
  }

  if(title.startsWith('Build the Model With Us')&&pageHero){
    const section=document.createElement('section');
    section.className='section border-top';
    section.innerHTML=`<div class="container"><p class="eyebrow">Governed participation</p><h2>Different input has a different role.</h2><div class="decision-flow"><div><b>Receive</b><small>Defined route</small></div><div><b>Classify</b><small>Experience / evidence / expertise / preference</small></div><div><b>Safety</b><small>Privacy screen</small></div><div><b>Verify</b><small>Where possible</small></div><div><b>Review</b><small>Route to the right question</small></div><div><b>Decide</b><small>Evidence thresholds apply</small></div><div><b>Show what changed</b><small>Close the loop</small></div></div></div>`;
    pageHero.after(section);
  }

  if(title.startsWith('Expert Review Network')&&pageHero){
    const section=document.createElement('section');
    section.className='section border-top';
    section.innerHTML=`<div class="container"><p class="eyebrow">Review process</p><h2>Credibility comes from scrutiny, not a wall of names.</h2><div class="decision-flow"><div><b>Question</b><small>Define issue</small></div><div><b>Expertise</b><small>Match reviewer</small></div><div><b>Credentials</b><small>Verify</small></div><div><b>Conflict check</b><small>Disclose</small></div><div><b>Review</b><small>Assess evidence</small></div><div><b>Dissent</b><small>Retain disagreement</small></div><div><b>Decision</b><small>Record model effect</small></div></div></div>`;
    pageHero.after(section);
  }

  if(title.startsWith('Sierra Leone baseline')&&pageHero){
    const section=document.createElement('section');
    section.className='section border-top';
    section.innerHTML=`<div class="container geo-panel"><div><p class="eyebrow">System geography</p><h2>Understand the system in space, not only on paper.</h2><p class="lead-sm">Verified maps should connect catchment responsibility, PHU access, referral distance and district structure.</p><p class="lab-note"><b>Boundary:</b> conceptual framework only. Real locations will appear only when the source data is verified.</p></div><div class="catchment" role="img" aria-label="Conceptual health-system geography"><span class="dot" style="left:20%;top:25%"></span><span class="dot" style="left:30%;top:70%"></span><span class="dot phu" style="left:50%;top:50%"></span><span class="dot" style="left:68%;top:28%"></span><span class="dot ref" style="left:80%;top:68%"></span></div></div>`;
    pageHero.after(section);
  }
})();